...
public:
    struct RaftPolygon {
        ...
        int layer_type; // SupporLayerType
        coordf_t height;
        const PrintObject* object;
    };
    std::map<coordf_t, std::vector<RaftPolygon>>          m_global_raft_polygons;
    std::mutex                                            m_global_raft_mutex;

    std::vector<std::unique_ptr<SupportLayer>>            m_merged_raft_layers;
    void                                                  make_merged_rafts();
