    ...
    SupportGeneratorLayerStorage& layer_storage)
{
    ...
    if (!intermediate_layers.empty() && support_params.has_interfaces()) {
        ...
        const auto minimum_island_radius_top    = support_params.support_material_interface_flow.scaled_spacing() /
                                                  support_params.top_interface_density;
        const auto minimum_island_radius_bottom = support_params.support_material_interface_flow.scaled_spacing() /
                                                  support_params.bottom_interface_density;
        ...
        auto insert_layer = [&layer_storage, smooth_supports, closing_distance, smoothing_distance, minimum_island_radius_top,
                             minimum_island_radius_bottom](SupportGeneratorLayer& intermediate_layer, Polygons& bottom, Polygons&& top,
            ...
            auto regularize = [&](Polygons polys, coordf_t minimum_island_radius) -> Polygons {
                if (polys.empty())
                    return polys;
                return smooth_supports ? smooth_outward(closing(std::move(polys), closing_distance + minimum_island_radius,
    ...
    for (size_t i = 0; i < layers_sorted.size();) {
        ...
        for (size_t u = i; u < j; ++u) {
            ...
            if (!layer.polygons.empty()) {
                empty                        = false;
                const bool is_base_interface = std::find(base_interface_layers.begin(), base_interface_layers.end(), &layer) !=
                                               base_interface_layers.end();
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, n_raft_layers), [&support_layers, &raft_layers, &intermediate_layers, &config,
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++support_layer_id) {
            ...
            if (support_layer_id > 0) {
                ...
                if (!to_infill_polygons.empty()) {
                    ...
                    fill_expolygons_with_sheath_generate_paths(
                        ...
                        filler,
                        config.raft_advanced_params ? float(config.raft_infill_density.value * 0.01) : float(support_params.support_density),
            ...
            if (support_layer_id == 0) {
                ...
                density = config.raft_advanced_params ? float(config.raft_interface_density.value * 0.01) :
                                                        float(support_params.raft_interface_density);
            } else
                continue;
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(n_raft_layers, support_layers.size()), [&config, &slicing_params, &support_params,
        ...
        auto filler_base_interface = std::unique_ptr<Fill>(
            base_interface_layers.empty() ?
                nullptr :
                Fill::new_from_type(support_params.top_interface_density > 0.95 || support_params.with_sheath ? ipRectilinear :
                                                                                                                ipSupportBase));
        ...
        for (size_t support_layer_id = range.begin(); support_layer_id < range.end(); ++support_layer_id) {
            ...
            if (organic_tree && support_params.zero_gap_interface_top && !support_params.can_merge_support_regions &&
                ...
                if (!interface_polygons.empty()) {
                    const coord_t trim_margin  = std::max(support_params.support_material_flow.scaled_width(),
                ...
                if (!layer_ex.empty() && !layer_ex.polygons_to_extrude().empty()) {
                    ...
                    bool bottom_interface = interface_layer_type == InterfaceLayerType::BottomContact ||
                                            (interface_layer_type == InterfaceLayerType::Interface &&
                                             layer_ex.layer->layer_type == SupporLayerType::BottomInterface);
